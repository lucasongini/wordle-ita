...
dizionarioDict = open('dizionario.txt', "r")
dizionario = dizionarioDict.readlines()
dizionarioDict.close()


# ricava il numero massimo di lettere possibili in una parola, controllando tutto l'elenco di parole
maxLenP = len(max(parole, key = len)) - 1
maxLenD = len(max(dizionario, key = len)) - 1
maxLen = maxLenP if maxLenP <= maxLenD else maxLenD
...
while True:
    ...
    if len(wordle) == maxN: # controlla che la parola sia della lunghezza richiesta
        if wordle.lower() + '\n' in dizionario: # aggiunge un '\n' alla fine della parola per poterla comparare con quelle nell'elenco e la converte in minuscolo
